...
> 마지막 업데이트: 2026-06-07 (강조 톤 완화: 차콜 블록 → 밝은 회색 `#E8E8E8` 채움 + 잉크
> 글자 + 헤어라인 보더. 검정 블록 풀채움 폐기)
...
2. **모노크롬 + 단일 포인트**: 그레이 단계 위주. 채움형 강조(버튼·활성 칩·"오늘")는 **밝은
   회색(`--color-accent #E8E8E8`) + 잉크 글자 + 1px 헤어라인**으로 인쇄물처럼 차분하게.
   얇은 차콜 액센트(포커스 보더 `focus:border-brand` + `ring-brand/15`, 선택일 링 `ring-brand`,
   텍스트 링크 `text-brand`)만 차콜로 유지. **검정 블록 풀채움(`bg-brand text-white`)은 금지**.
...
| 토큰 | 값 | 용도 |
|------|-----|------|
| `--paper` | `#F5F5F4` | 페이지 배경(옅은 중립 회색) |
| `--surface` | `#FFFFFF` | 카드/모달 표면(순백) |
| `--ink` | `#1A1A1A` | 본문 잉크(따뜻한 차콜) |
| `--color-brand` | `#111111` | 얇은 강조(포커스 보더·선택 링·텍스트 링크). Tailwind `brand` 유틸 |
| `--color-brand-50` | `#f7f7f8` | 옅은 채움(브랜드 버튼 hover 배경) |
| `--color-brand-100` | `#f2f2f2` | 옅은 채움(강조 배경) |
| `--color-accent` | `#E8E8E8` | **채움형 강조(기본 버튼·활성 카테고리 칩·"오늘" 마커)**. 잉크 글자 + 헤어라인과 함께 사용 |
| `--color-accent-hover` | `#DEDEDE` | accent 버튼 hover |
...
- **로고**: `public/logo.svg`·`public/icon.svg` 캘린더 본체는 파스텔블루 `#A8C8EF`(흰 디바이더·노브
  유지). 로고는 브랜드 마크라 모노크롬 규칙의 예외.
...
| 컴포넌트 | 스펙 |
|----------|------|
| 카드(월 그리드·로그인) | `bg-[var(--surface)]`(순백) + 헤어라인, `rounded-none`, 그림자 없음 |
| 기본 버튼 | `bg-accent text-[var(--ink)]` + 1px 헤어라인(`border-[var(--border)]`), `rounded-none`, weight 600~700, hover `bg-accent-hover` |
| 보조/취소 버튼 | `bg-gray-100 text-gray-700`, `rounded-none` |
| 아웃라인 보조(가져오기 등) | `bg-white text-[var(--ink)]` + 헤어라인, hover `bg-gray-50` |
...
| 카테고리 칩 | `rounded-none`. **선택**=`bg-accent text-[var(--ink)]` + 헤어라인 + 좌측 사각 도트(카테고리색). **미선택**=`bg-transparent text-gray-400` + 좌측 사각 도트(카테고리색). 둘 다 모노톤(칩 배경은 카테고리색 채우지 않음) |
...
| "오늘" 마커 | `bg-accent text-[var(--ink)]` 사각 채움 + 헤어라인(`h-5 w-5 rounded-none border border-[var(--border)]`) |
...
**Do**: 페이퍼 + 서피스 + 잉크 토큰 사용, 헤어라인 보더, **밝은 회색(`#E8E8E8`) 강조**,
얇은 차콜 액센트(포커스·선택 링·텍스트 링크), tabular 숫자, 넉넉한 여백, 굵기·크기로
위계(Pretendard 단일), 라인 아이콘, **완전 각짐(rounded-none)**.

**Don't**: 그라데이션, **둥근 모서리(`rounded-*` 일체, `rounded-full` 도트 포함)**, 무거운
그림자(시트 제외), **검정 블록 풀채움 강조(`bg-brand text-white`/`bg-[var(--ink)] text-white`)**,
크롬에 블루/앰버, 장식 색, 유니코드 글리프/이모지 아이콘, 컬러 타일 배경, **칩 배경을 카테고리색으로
채우기**, 누런/웜 톤 배경, 세리프/디스플레이 서체(Pretendard 단일 유지).
