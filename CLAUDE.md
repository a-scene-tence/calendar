...
| 2026-06-07 | 차콜 블록(`bg-brand text-white`) 강조가 시각적으로 무겁고 에디토리얼 톤에서 이질적 | 단일 포인트가 검정 풀채움이라 인쇄물 같은 차분한 위계가 안 살고 버튼/칩/오늘 마커가 페이지를 누름 | `--color-accent #E8E8E8`/`--color-accent-hover #DEDEDE` 토큰 신설 + 6개 채움 버튼·활성 칩·"오늘" 마커를 `bg-accent text-[var(--ink)] border-[var(--border)]` 패턴으로 전환. 얇은 차콜 액센트(`focus:border-brand`·`ring-brand`·`text-brand`)만 유지 | 채움형 강조는 검정 블록 대신 밝은 회색+잉크+헤어라인. 시각 토큰 변경 시 `design.md` 동기 갱신 |
...
| 2026-06-09 | Vercel에서 "오늘 날짜가 안 맞음"(하루 빠름) | `CalendarMonth`(use client)도 Next App Router가 초기 HTML을 서버(Vercel=UTC)에서 SSR. KST 자정~오전9시엔 서버 `new Date()`가 전날 → `viewDate`/`selectedDay`/`todayKey`가 하루 어긋나고 하이드레이션 불일치 | "오늘" 계산을 `seoulTodayKey()`(`Intl.DateTimeFormat("en-CA",{timeZone:"Asia/Seoul",...})`)로 KST 고정 — 서버/브라우저 무관 결정적 | 날짜 "오늘" 기준은 `new Date()` 로컬 추출 금지. SSR 서버 TZ(UTC) 영향 — 타임존 명시(`Intl` timeZone). 이벤트 시각이 이미 Asia/Seoul이므로 "오늘"도 동일 기준 |
| 2026-06-09 | 구글에서 추가된 카테고리(재경본부)를 Vercel 관리에서 숨길 수 없음 | `app/api/calendar/route.ts`가 `EXCLUDED_NAMES=Set(["재경본부"])`로 서버에서 **이름 하드코딩 완전 제외** → 칩·관리 모달 어디에도 안 뜸. 관리 모달은 `ownerCategories`(소유만)만 받아 숨김 토글도 없음 | 하드코딩 제외 삭제 + `hidden_categories` 컬럼/`hiddenCategories` 영속 설정 추가. 관리 모달이 전체 카테고리(accessRole 포함)를 받아 눈 토글로 숨김(화면에서만, 검색엔 포함). 소유 캘린더만 수정/삭제 노출 | 카테고리 숨김은 코드 하드코딩 말고 **사용자 설정(Supabase user_tokens + localStorage 캐시)**으로. 새 prefs 컬럼은 GET에서 기존 컬럼과 **독립 조회**(미적용 시 categoryOrder 동반 붕괴 방지) |
| 2026-06-10 | "오늘" 마커가 어제로 표시되는 회귀(이전 `seoulTodayKey` 수정에도 불구) | 홈 `/`가 `○ Static` 프리렌더 + `CalendarMonth`에 loading early-return 부재 → 그리드가 **빌드 시점에 렌더**돼 `todayKey`가 정적 HTML로 굳음. PWA/SW(Serwist)·CDN가 그 정적 `/`를 서빙하면 사용자는 빌드 날짜를 본다 | `app/page.tsx`에 `export const dynamic = "force-dynamic"` 추가(빌드 표 `ƒ /`로 바뀌어 정적 고정 제거) + `CalendarMonth`의 `todayKey`를 state로 바꾸고 `visibilitychange`/`focus`에 `seoulTodayKey()` 재계산 | 날짜 의존 UI 라우트는 **정적 프리렌더 금지**. SSR로 충분치 않다면 클라에서 마운트·복귀 시 한 번 더 보정. `viewDate`까지 자동 점프시키지는 말 것(사용자 보기 흐름 끊김) |
