...
| 2026-06-06 | 1차 모노크롬 전환만으론 에디토리얼 톤이 약함(산세리프 일색·시스템 그레이 배경·둥근 모서리가 잡지 인쇄물 톤을 희석) | 디자인 토큰이 표면/잉크 분리 없이 그레이만 사용. 디스플레이 폰트 슬롯 없음. 라디우스 스케일이 4/8/16로 살아 있음 | `--paper #F4EEE2` + `--surface #FBF8F2` + `--ink #1A1A1A` 신설, `--font-display`(Cormorant Garamond + Noto Serif KR + Pretendard 폴백) 도입, 모든 `rounded-*` → `rounded-none`(도트·핸들·"오늘" 포함), 카테고리 칩은 모노톤(도트만 색) | 큰 톤 변경은 토큰 단에서 처리하고 컴포넌트는 토큰만 참조 — 시각 토큰은 `design.md` 단일 출처. Tailwind v4 `@theme`에 `--font-display`도 등록 가능(`font-display` 유틸 또는 `.font-display` 베이스 클래스로 통일) |
